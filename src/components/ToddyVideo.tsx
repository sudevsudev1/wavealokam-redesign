...
const ToddyVideo = () => {
  ...
  // ScrollTrigger for Toddy video (Activity 6)
  // Starts before Mangrove Adventure (Activity 5) disappears
  useEffect(() => {
    ...
    const videoTrigger = ScrollTrigger.create({
      ...
      // Activity 6 is at ~62.5-75% of total scroll (activities.length = 8)
      // Start a few frames before Activity 5 ends (around 60% of total)
      start: () => `top+=${window.innerHeight * 4.85} top`,
      // End when Toddy section rolls out (around 77%)
      end: () => `top+=${window.innerHeight * 6.2} top`,
      scrub: true,
      onUpdate: (self) => {
