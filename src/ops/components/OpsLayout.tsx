      ...
      <header className="sticky top-0 z-50 bg-background border-b border-border">
        <div className="flex items-center justify-between px-3 h-12">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[10px] text-muted-foreground capitalize shrink-0">({profile.role})</span>
          </div>
          ...
          <nav className="lg:hidden border-t border-border bg-background px-2 py-1.5 space-y-0.5">
            {items.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-2 px-3 py-2.5 rounded-md text-xs font-medium
